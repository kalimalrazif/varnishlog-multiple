...
$pidfile = File::Pid->new({
   file => '/var/run/varnishlog_multiple.pid'
});
...
$varnish_log_feed=File::Tail->new($varnish_log_file );

print $mi_log "Empezando a procesar el log\n";

while(defined(my $linea = $varnish_log_feed->read)) {
	my ($host, @resto) = split / /, $linea;
	$host =~ s/www\.//;
	$host = lc $host;
...
sub open_log_files {
	foreach my $dominio (@dominios) {
		...
		die "$archivo no pudo ser abierto" if (!defined $log_files->{$dominio});
